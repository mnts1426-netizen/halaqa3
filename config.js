...
// 7. إعدادات OneSignal لتفعيل إشعارات Push الفورية للمديرة
// ONESIGNAL_APP_ID: قيمة عامة غير حساسة، لا مشكلة من بقائها هنا
// TODO: يجب إنشاء تطبيق OneSignal مستقل خاص بموقع الدار النسائية (دومين منفصل عن حلقات الهدى)
// ثم لصق الـ App ID الخاص به هنا. لا يصح استخدام نفس معرّف تطبيق الحلقات الرجالية
// لأن تطبيقات OneSignal مسجّلة لكل دومين/موقع على حدة. طالما بقيت هذه القيمة فارغة
// فإن ميزة إشعارات Push ستبقى معطّلة بأمان دون أي أخطاء (راجع isOneSignalConfigured
// في pwa-notifications.js).
window.ONESIGNAL_APP_ID = "";

// تنبيه أمني مهم: لا يجب أبداً وضع ONESIGNAL_REST_API_KEY في هذا الملف لأنه يظهر
// مباشرة لأي زائر للموقع (حتى بدون تسجيل دخول) عبر "عرض مصدر الصفحة" - وهو مفتاح سري
// خطير يسمح لأي شخص يحصل عليه بإرسال إشعارات مزيفة لجميع مستخدمي التطبيق أو
// الوصول لبيانات المشتركين. عند الحاجة لإرسال إشعارات فعلية يجب توليد مفتاح REST API
// واستخدامه فقط من خلال خادم وسيط (Cloud Function) وليس مباشرة من كود المتصفح.
